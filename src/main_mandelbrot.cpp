...
//    float sizeX = 2.0f;
    ...
    context.activate();
    {
        ...
        bool printLog = false; // Смог. Показал 8
        kernel.compile(printLog);

        gpu::gpu_mem_32f res_buff;
        res_buff.resizeN(width * height);
        ...
        timer t;
        for (int i = 0; i < benchmarkingIters; ++i) {
            kernel.exec(gpu::WorkSize(workGroupSizeX, workGroupSizeY, width, height),
                        res_buff,
                        width,
                        height,
                        centralX - sizeX / 2.0f,
        ...
        res_buff.readN(gpu_results.ptr(), width * height);

        // Копируем код и меняем CPU -> GPU
        size_t flopsInLoop = 10;
        size_t maxApproximateFlops = width * height * iterationsLimit * flopsInLoop;
        size_t gflops = 1000*1000*1000;
